...
def resize(image, target, size, max_size=None):
    ...
    if "sboxes" in target:
        ...
        scaled_boxes = boxes \
            * torch.as_tensor([ratio_width, ratio_height, ratio_width, ratio_height])
        target["sboxes"] = scaled_boxes

    if "oboxes" in target:
        boxes = target["oboxes"]
        scaled_boxes = boxes \
            * torch.as_tensor([ratio_width, ratio_height, ratio_width, ratio_height])
        target["oboxes"] = scaled_boxes

    if "sarea" in target:
        area = target["sarea"]
        scaled_area = area * (ratio_width * ratio_height)
        target["sarea"] = scaled_area

    if "oarea" in target:
        area = target["oarea"]
        scaled_area = area * (ratio_width * ratio_height)
        target["oarea"] = scaled_area
    ...
    def __call__(self, images, targets=None):
        ...
        for i, target in enumerate(targets):
            if "sboxes" in target:
                ...
                boxes = box_xyxy_to_cxcywh(boxes)
                boxes = boxes / torch.tensor([w, h, w, h], dtype=torch.float32)
                targets[i]["sboxes"] = boxes
            if "oboxes" in target:
                boxes = target["oboxes"]
                boxes = box_xyxy_to_cxcywh(boxes)
                boxes = boxes / torch.tensor([w, h, w, h], dtype=torch.float32)
                targets[i]["oboxes"] = boxes
            
        return images, targets


class DebugBatchNormalize:
    def __init__(self):
        super().__init__()
    ...
    def __call__(self, imgs, targets):
        if random.random() < self.p:
            ...
            for i in range(len(imgs)):
                img, target = hflip(imgs[i], targets[i])
                _imgs.append(img)
                _targets.append(target)
            
            return _imgs, _targets
        return imgs, targets
...
class BatchRandomResize:
    ...
    def __call__(self, imgs, targets=None):
        ...
        _imgs, _targets = [], []
        for i in range(len(imgs)):
            img, target = resize(imgs[i], targets[i], size, self.max_size)
            _imgs.append(img)
            _targets.append(target)
...
def vrd_crop(image, target, region):
    i, j, h, w = region
    target = target.copy()
    ...
    #fields = ["labels", "area", "iscrowd", "ignore", "track_ids"]
    fields = ["sclasses", "oclasses", "so_traj_ids", "svo_ids", "sarea", "oarea"]  # num_svo
    # verb_classes
    
    orig_s_area = target["sarea"]
    orig_o_area = target["oarea"]
    ...
    if "sboxes" in target:
        ...
        cropped_boxes = cropped_boxes.clamp(min=0)
        area = (cropped_boxes[:, 1, :] - cropped_boxes[:, 0, :]).prod(dim=1)
        target["sboxes"] = cropped_boxes.reshape(-1, 4)
        target["sarea"] = area
        fields.append("sboxes")
    ...
    if "oboxes" in target:
        ...
        cropped_boxes = cropped_boxes.clamp(min=0)
        area = (cropped_boxes[:, 1, :] - cropped_boxes[:, 0, :]).prod(dim=1)
        target["oboxes"] = cropped_boxes.reshape(-1, 4)
        target["oarea"] = area
        fields.append("oboxes")

    if "sboxes" in target or "oboxes" in target:
        # new area must be at least % of orginal area
        
        #print(target["sarea"].shape, target["oarea"].shape, orig_s_area.shape, orig_o_area.shape)

        keep = (target["sarea"] >= orig_s_area * 0.2) & (target["oarea"] >= orig_o_area * 0.2)
        for field in fields:
            if field in target:
                target[field] = target[field][keep]
        ...
        verb_classes = target["verb_classes"]
        raw_verb_classes = target["raw_verb_classes"]
        target["verb_classes"] = [verb_classes[i] for i, flag in enumerate(keep.tolist()) if flag]
        target["raw_verb_classes"] = [raw_verb_classes[i] for i, flag in enumerate(keep.tolist()) if flag]
        
    target["num_svo"] = target["so_traj_ids"].shape[0]
    
    return cropped_image, target, False


class BatchRandomSizeCrop:
    def __init__(self,
                 min_size: Union[tuple, list, int],
                 max_size: Union[tuple, list, int] = None):
        if isinstance(min_size, int):
    ...
    def __call__(self, imgs, targets):
        ...
        if self.max_size is None:
            w = random.randint(min(self.min_size[0], width), width)
            h = random.randint(min(self.min_size[1], height), height)
        else:
            w = random.randint(
                min(self.min_size[0], width),
                min(width, self.max_size[0]))
            h = random.randint(
                min(self.min_size[1], height),
                min(height, self.max_size[1]))
        
        _imgs, _targets = [], []
        for i in range(len(imgs)):
            region = T.RandomCrop.get_params(imgs[i], [h, w])
            img, target, no_pos = vrd_crop(imgs[i], targets[i], region)
            if no_pos:
                return imgs, targets
            _imgs.append(img)
            _targets.append(target)
